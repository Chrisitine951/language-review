...
// ============================================================
// 18 週備考大表（v2.1.0 建立；v2.2.7 依實際進度重排；v2.2.8 加入緩衝機制）
// 起算 2026-07-20（週一）→ 考試 2026-11-21（週六）＝ 共 18 週，目標總分 7.0
//
// 【v2.2.8 為什麼要加緩衝｜這版最重要的改動】
//   v2.2.7 排完後試算過每週工時：第三階段每週需要 13.5–17.5 小時，但她實際可用
//   只有約 14.25 小時——**在「一切順利」的前提下就已經超載**，而且完全沒有空間給
//   加班、生病、朋友約、假日出遊，或某一回檢討特別卡。只要一週出狀況就開始積壓，
//   然後會誤以為是自己跟不上，其實是表排錯了。三個對策：
//   1. **週日一律是機動補做日**：不排新內容，只有「每週回顧＋補做」。這週順利就休息，
//      有落後就補。等於每週內建 2–3 小時緩衝。⚠️ 這格刻意不放內容，**未來調整大表時
//      不要為了多塞一項而佔用它**，那會直接讓整份計畫回到無緩衝的脆弱狀態。
//      （她週六常有出遊，週六那 3 小時大塊本來就不保險，週日就是它的保險。）
//   2. **檢討深度隨階段遞減**：W4–9 是診斷期，還在找自己的錯誤模式，值得走完整六步驟；
//      W10 起已經知道常錯什麼，改成「聽力只精講錯題＋重聽一次」「閱讀只對錯題和
//      『答對但用猜的』做拆解檢核」。每週省 2–3 小時，而且教學上本來就該這樣收斂。
//   3. **閱讀早期 2 篇/週**（不是一晚兩篇，那塞不下），W10 起改成整回限時＋精簡檢討。
//   代價：材料量從 14 回降到約 9 回真題＋4 次模考，劍13/14/15/20 這次不排。
//   取捨理由：9 回做得完整踏實 > 14 回做得草率，她的方法本來就重檢討不重刷題量。
//
// 【內容來源】Christine 的備考指南＋她本人 2026-07-28 的進度調整，
//   已依她的實際生活調整：
//   - 平日下班約 19:00、約 00:00 睡 → 每天約 2–2.5 小時
//   - 週三晚上 Cambly（口說），課後還有約 1.5 小時
//   - ⚠️ 週四晚上 21:00 有德文家教 1.5 小時 → 週四「不排任何英文任務」
//   - ⚠️ **聽力完整一回排在週六**：六步驟檢討流程實測約 2.5–3 小時，平日晚上塞不下，
//     且步驟③⑥要「趁記憶還熱時再聽」，拆兩天會削弱設計用意，所以整回放週六。
//   - ⚠️ **週日只放機動補做**（見上方對策 1）
//
// 【五階段】W1–3 方法建立（讀完平行閱讀法，不做題）→ W4–9 應用方法・完整檢討（診斷期，
//           不限時）→ W10–13 限時＋精簡檢討 → W14–17 模考衝刺 → W18 考前週
//
// 【材料分配】閱讀：劍10 T1–T4（W4–9）→ 劍11 T1–T3（W10–12）→ 劍12 T1–T2（W14–15）；
//   聽力：劍10 T1–T4（W3–6）→ 劍11 T1–T4（W7–10）→ 劍12 T1–T3（W11–15）；
//   模考 4 次用劍18 T1/T2、劍19 T1/T2。劍13、劍14、劍15、劍20 這次不排（留作備用）。
//
// 【每週固定習慣】（背景習慣，不逐日列進計畫格，避免灌爆畫面）
//   - 每天：英文日記 3 句（記錄頁日記分頁有每日題目，從文法/口說/閱讀隨機出）
//   - 每天：EGIU 1 單元（文法頁棋盤打卡；落後時可用週日機動格補）
//   - 每週：寫作範文＋口說範例回答各 1–2 篇背誦/解析（雅思頁各自有紀錄區塊）
//
...
//   ⚠️ **改這份大表不會動到「已經帶入過」的週**——要套用新版本，得到首頁按
//   「🔄 重置本週」（`resetWeekPlan()`）強制清掉重帶。還沒帶入的未來週則會直接生效。
//
...
const IELTS_STUDY_PLAN = [
  // ===== 第一階段：方法建立（W1–3）——讀完《平行閱讀法》，先不做題 =====
  { week: '2026-07-20', no: 1, phase: '第一階段・方法建立（W1–3）', egiu: 'Unit 1–9', items: [
    ...
    ['writing', 6, '🔄 機動日：每週回顧＋補做落後的；沒落後就休息'],
    ['material', 6, 'EGIU Unit 1–9（每日1單元）'],
  ]},
  { week: '2026-07-27', no: 2, phase: '第一階段・方法建立（W1–3）', egiu: 'Unit 10–18', items: [
    ['reading', 0, '平行閱讀法 第二章（填空題）精讀'],
    ['reading', 1, '平行閱讀法 第二章 其餘題型'],
    ['speaking', 2, 'Cambly＋口說範例回答解析 1 篇（記進「每週範文紀錄」）'],
    ['writing', 4, '寫作範文背誦/解析 1 篇（記進「每週範文紀錄」）'],
    ['reading', 5, '平行閱讀法 第三章（五步驟方法論）精讀'],
    ['writing', 6, '🔄 機動日：每週回顧＋補做落後的；沒落後就休息'],
    ['material', 6, 'EGIU Unit 10–18'],
  ]},
  { week: '2026-08-03', no: 3, phase: '第一階段・方法建立（W1–3）', egiu: 'Unit 19–27', items: [
    ['reading', 0, '平行閱讀法 讀完剩餘章節'],
    ['reading', 1, '平行閱讀法 全書重點整理（五步驟／三重現／七類信號詞）'],
    ['speaking', 2, 'Cambly＋口說範例回答解析 第2篇'],
    ['writing', 4, '寫作範文背誦/解析 第2篇'],
    ['listening', 5, '【暖身】劍10 T1 聽力完整一回＋六步驟檢討（約3小時）'],
    ['writing', 6, '🔄 機動日：每週回顧（讀完了！下週開始做題）'],
    ['material', 6, 'EGIU Unit 19–27'],
  ]},

  // ===== 第二階段：應用方法・完整檢討（W4–9）——診斷期，不限時，閱讀 2 篇/週 =====
  { week: '2026-08-10', no: 4, phase: '第二階段・應用方法・完整檢討（W4–9）', egiu: 'Unit 28–36', items: [
    ['reading', 0, '劍10 T1 P1 不限時＋拆解檢核（每題都過）'],
    ['reading', 1, '劍10 T1 P2 不限時＋拆解檢核'],
    ['speaking', 2, 'Cambly＋語料整理'],
    ['writing', 4, '寫作範文背誦/解析 1 篇'],
    ['listening', 5, '劍10 T2 聽力完整一回＋六步驟檢討'],
    ['writing', 6, '🔄 機動日：每週回顧＋補做落後的；沒落後就休息'],
    ['material', 6, 'EGIU Unit 28–36'],
  ]},
  { week: '2026-08-17', no: 5, phase: '第二階段・應用方法・完整檢討（W4–9）', egiu: 'Unit 37–45', items: [
    ['reading', 0, '劍10 T1 P3 不限時＋拆解檢核'],
    ['reading', 1, '劍10 T2 P1 不限時＋拆解檢核'],
    ['speaking', 2, 'Cambly＋建立第1份核心語料（書／滑雪…）'],
    ['writing', 4, 'Simon Task 2 結構複習＋範文解析 1 篇'],
    ['listening', 5, '劍10 T3 聽力完整一回＋檢討'],
    ['writing', 6, '🔄 機動日：每週回顧＋補做落後的'],
    ['material', 6, 'EGIU Unit 37–45'],
  ]},
  { week: '2026-08-24', no: 6, phase: '第二階段・應用方法・完整檢討（W4–9）', egiu: 'Unit 46–54', items: [
    ['reading', 0, '劍10 T2 P2 不限時＋拆解檢核'],
    ['reading', 1, '劍10 T2 P3 不限時＋拆解檢核'],
    ['speaking', 2, 'Cambly＋核心語料第2份'],
    ['writing', 4, '【第一篇】Task 2 不限時（plan 10分鐘要做滿）'],
    ['listening', 5, '劍10 T4 聽力完整一回＋檢討'],
    ['writing', 6, '🔄 機動日：Task 2 訂正＋每週回顧（訂正比多寫一篇有效）'],
    ['material', 6, 'EGIU Unit 46–54'],
  ]},
  { week: '2026-08-31', no: 7, phase: '第二階段・應用方法・完整檢討（W4–9）', egiu: 'Unit 55–63', items: [
    ['reading', 0, '劍10 T3 P1 不限時＋拆解檢核'],
    ['reading', 1, '劍10 T3 P2 不限時＋拆解檢核'],
    ['speaking', 2, 'Cambly＋自錄15分鐘'],
    ['writing', 4, 'Task 1 結構＋圖表詞彙＋範文解析 1 篇'],
    ['listening', 5, '劍11 T1 聽力完整一回＋檢討'],
    ['writing', 6, '🔄 機動日：每週回顧＋補做落後的'],
    ['material', 6, 'EGIU Unit 55–63'],
  ]},
  { week: '2026-09-07', no: 8, phase: '第二階段・應用方法・完整檢討（W4–9）', egiu: 'Unit 64–72', items: [
    ...
    ['writing', 6, '🔄 機動日：Task 1 訂正＋每週回顧'],
    ['material', 6, 'EGIU Unit 64–72'],
  ]},
  { week: '2026-09-14', no: 9, phase: '第二階段・應用方法・完整檢討（W4–9）', egiu: 'Unit 73–81', items: [
    ['reading', 0, '劍10 T4 P2 不限時＋拆解檢核'],
    ['reading', 1, '劍10 T4 P3＋劍10 全書錯題回顧'],
    ['speaking', 2, 'Cambly＋自錄15分鐘×2'],
    ['writing', 4, 'Task 2 一篇 60分鐘內＋範文解析'],
    ['listening', 5, '劍11 T3 聽力完整一回＋檢討'],
    ['writing', 6, '🔄 機動日：階段回顧——劍10 做完，列出四科弱點清單'],
    ['material', 6, 'EGIU Unit 73–81'],
  ]},

  // ===== 第三階段：限時＋精簡檢討（W10–13）——檢討開始收斂，只追錯題 =====
  { week: '2026-09-21', no: 10, phase: '第三階段・限時＋精簡檢討（W10–13）', egiu: 'Unit 82–90', items: [
    ['reading', 0, '劍11 T1 閱讀整回 60分鐘限時（首次全限時）'],
    ['reading', 1, '劍11 T1 檢討：只對錯題＋「答對但用猜的」做拆解檢核'],
    ['speaking', 2, 'Cambly＋核心語料第4份'],
    ['writing', 4, 'Task 1 30分鐘限時'],
    ['listening', 5, '劍11 T4 聽力整回30分＋精簡檢討（只精講錯題＋重聽一次）'],
    ['writing', 6, '🔄 機動日：每週回顧＋補做落後的'],
    ['material', 6, 'EGIU Unit 82–90'],
  ]},
  { week: '2026-09-28', no: 11, phase: '第三階段・限時＋精簡檢討（W10–13）', egiu: 'Unit 91–99', items: [
    ['reading', 0, '劍11 T2 閱讀整回 60分鐘限時'],
    ['reading', 1, '劍11 T2 檢討（只錯題＋猜對的）'],
    ['speaking', 2, 'Cambly＋口說模考（Part 1–3 全程錄音）'],
    ['writing', 4, 'Task 2 40分鐘限時（首次）'],
    ['listening', 5, '劍12 T1 聽力整回30分＋精簡檢討'],
    ['writing', 6, '🔄 機動日：Task 2 訂正重寫＋每週回顧'],
    ['material', 6, 'EGIU Unit 91–99'],
  ]},
  { week: '2026-10-05', no: 12, phase: '第三階段・限時＋精簡檢討（W10–13）', egiu: 'Unit 100–108', items: [
    ['reading', 0, '劍11 T3 閱讀整回 60分鐘限時'],
    ['reading', 1, '劍11 T3 檢討（只錯題＋猜對的）'],
    ['speaking', 2, 'Cambly＋弱點題型口說練習'],
    ['writing', 4, '模板總複習（不寫新的，為明天模考留體力）'],
    ['reading', 5, '【模考1】劍18 T1 聽力＋閱讀＋寫作連做（照考試時序）'],
    ['writing', 6, '🔄 機動日：模考1 檢討（四科弱點清單）＋成績記進練習紀錄'],
    ['material', 6, 'EGIU Unit 100–108'],
  ]},
  { week: '2026-10-12', no: 13, phase: '第三階段・限時＋精簡檢討（W10–13）', egiu: 'Unit 109–117', items: [
    ['reading', 0, '模考1 弱題型專練'],
    ['listening', 1, '模考1 聽力錯因專練＋場景詞'],
    ['speaking', 2, 'Cambly＋自錄×2'],
    ['writing', 4, 'Task 2 弱題型一篇 40分鐘'],
    ['listening', 5, '劍12 T2 聽力整回30分＋精簡檢討'],
    ['writing', 6, '🔄 機動日：每週回顧＋補做落後的'],
    ['material', 6, 'EGIU Unit 109–117'],
  ]},

  // ===== 第四階段：模考衝刺（W14–17）——隔週一次全模考 =====
  { week: '2026-10-19', no: 14, phase: '第四階段・模考衝刺（W14–17）', egiu: 'Unit 118–126', items: [
    ['reading', 0, '劍12 T1 閱讀整回 60分鐘限時'],
    ['reading', 1, '劍12 T1 檢討（只錯題＋猜對的）'],
    ['speaking', 2, 'Cambly＋口說模考第2次'],
    ['writing', 4, 'Task 1 各圖表模板總複習（不寫新的）'],
    ['reading', 5, '【模考2】劍18 T2 全科連做'],
    ['writing', 6, '🔄 機動日：模考2 檢討＋每週回顧'],
    ['material', 6, 'EGIU Unit 118–126'],
  ]},
  { week: '2026-10-26', no: 15, phase: '第四階段・模考衝刺（W14–17）', egiu: 'Unit 127–135', items: [
    ['reading', 0, '劍12 T2 閱讀整回 60分鐘限時'],
    ['reading', 1, '劍12 T2 檢討＋模考2 弱點專練'],
    ['speaking', 2, 'Cambly＋串題總演練（一份語料對五題）'],
    ['writing', 4, 'Task 2 高頻主題 plan 練習×3（只 plan 不寫全篇）'],
    ['listening', 5, '劍12 T3 聽力整回30分＋精簡檢討'],
    ['writing', 6, '🔄 機動日：每週回顧＋補做落後的'],
    ['material', 6, 'EGIU Unit 127–135'],
  ]},
  { week: '2026-11-02', no: 16, phase: '第四階段・模考衝刺（W14–17）', egiu: 'Unit 136–145', items: [
    ['reading', 0, '弱題型專練（依模考2 弱點清單）'],
    ['listening', 1, '聽力錯因總瀏覽＋場景詞掃讀'],
    ['speaking', 2, 'Cambly＋考場流程演練'],
    ['writing', 4, 'Task 2 高頻主題 plan 練習×3（不寫全篇）'],
    ['reading', 5, '【模考3】劍19 T1 全科連做'],
    ['writing', 6, '🔄 機動日：模考3 檢討＋每週回顧'],
    ['material', 6, 'EGIU Unit 136–145（週末複習）→ 145 全部完成！'],
  ]},
