...
// ============================================================
// 18 週備考大表（v2.1.0 建立；v2.2.7 重排；v2.2.8 加緩衝；v2.4.0 依實際進度大改）
// 起算 2026-07-20（週一）→ 考試 2026-11-21（週六）
//
// 【v2.4.0 改了什麼｜2026-08-04 依 Christine 實際狀況重排 W3 之後】
//   1. **EGIU 改成精選 98 個單元，不做完 145**（最重要的改動，見下方「EGIU 取捨」）
//   2. **口說改成週一＋週三各 30 分鐘**（Cambly 找到專門教口說的老師，兩次短課比
//      一次一小時不累）。原本只有週三一格。
//   3. **寫作分兩階段**：W3–W7 只做「範文分析＋抽語料＋plan 練習」，W8 起才寫完整篇。
//      ⚠️ 刻意不採用「背整篇範文」——考官對記憶痕跡敏感，而且背下來換題目就套不上。
//      改成抽「主題詞彙／搭配詞／句型骨架」進 App 的 Task 2 佳句，這些才能跨題目重用。
//   4. 閱讀進度符合預期（W3 讀完平行閱讀法，W4 開始劍10），聽力 W3 第一次完整練習。
//
// 【EGIU 取捨｜為什麼不做完 145】
//   實測速度：7/19 起兩週多完成 13 個單元 ≈ 6–7 個/週。
//   剩餘 132 個 ÷ 可排新單元的 14 週（W3–W16）= 需要 9.4/週，遠超她的實際能力。
//   硬排的下場就是每週都在追進度、每週都覺得自己落後（v2.2.8 已經因為同樣的錯誤重排過一次）。
//   → 決定：**維持 7/週，砍掉 34 個對雅思寫作幫助最小的單元**，只做 98 個。
//   砍掉的（依她 App 內累積的錯誤類型排優先序，保留跟她真實弱點對得上的）：
//     137–145 片語動詞全段（9）：偏口語，雅思寫作用正式動詞反而得分
//     123–127 位置介系詞（5）：空間介系詞在學術寫作幾乎不會失分
//     101–104 hard/hardly、so/such、enough/too、quite/rather（4）：語感細節
//     109–112 語序、still/yet/already、even（4）：副詞位置細則，效益低
//     114/117/118 in case、like and as、as if（3）
//     47–48 轉述句（2）：Task 2 用不到
//     51–52 助動詞短答、附加問句（2）：純口語文法（49–50 問句保留，她錯過兩次）
//     46 have something done、59 prefer/would rather、61 be/get used to、
//     63 there's no point in -ing、91 each and every（各 1）
//   完整保留：14–45 全部時態/情態/條件/被動（她最大弱點）、53–58 及 60/62/64–68
//     動詞句型、69–90 冠詞名詞代名詞、92–100 關係子句、105–108 比較級最高級
//     （Task 1 圖表一定用到）、113/115/116/119/120 連接詞、
//     121–122 及 128–136 介系詞（她長期的 to vs for 問題就在這）
//   ⚠️ 98 個 ÷ 14 週 = 剛好 7/週，**零緩衝**。落後時用週日機動日補；真的追不上就
//     再砍保留清單裡的低優先單元（98–100、89–90、64–68），不要壓縮休息時間。
//
// 【依她的實際作息排】
//   - 平日下班約 19:00、約 00:00 睡 → 每天約 2–2.5 小時
//   - 週一、週三：Cambly 口說課各 30 分鐘（課後還有時間排別的）
//   - ⚠️ 週四晚上 21:00 德文家教 → 週四「不排任何英文任務」
//   - ⚠️ **聽力完整一回排在週六**：六步驟檢討實測 2.5–3 小時，平日晚上塞不下，
//     且步驟③⑥要「趁記憶還熱時再聽」，拆兩天會削弱設計用意
//   - ⚠️ **週日只放機動補做**：這格刻意不放內容，調整大表時不要為了多塞一項而佔用它，
//     那會讓整份計畫回到「零緩衝」的脆弱狀態。她週六常有出遊，週日就是週六的保險。
//
// 【五階段】W1–3 方法建立（讀完平行閱讀法）→ W4–9 應用方法・完整檢討（診斷期，不限時）
//           → W10–13 限時＋精簡檢討 → W14–17 模考衝刺 → W18 考前週
//
// 【材料分配】閱讀：劍10 T1–T4（W4–9）→ 劍11 T1–T3（W10–12）→ 劍12 T1–T2（W14–15）
//   聽力：劍10 T1–T4（W3–6）→ 劍11 T1–T4（W7–10）→ 劍12 T1–T3（W11–15）
//   模考 4 次：劍18 T1/T2、劍19 T1/T2。劍13–15、劍20 不排（留備用）
//
// 【每週固定習慣】（背景習慣，不逐日列進計畫格，避免灌爆畫面）
//   - 每天：英文日記 3 句（記錄頁有每日題目，從文法/口說/閱讀隨機出）
//   - 每天：EGIU 1 個單元（文法頁棋盤打卡）
//   - 每週：寫作範文分析＋口說範例回答解析（雅思頁各自有紀錄區塊）
//
// 【使用方式】週計畫表第一次開啟「本週或未來週」時自動帶入（標 fromPlan），打勾即可。
//   已帶入的週記在 planImportedWeeks。⚠️ **改這份大表不會動到已帶入過的週**——
//   要套用新版本，到首頁按「🔄 重置本週」（resetWeekPlan()）強制清掉重帶。
//
// 欄位：week=該週週一日期、no=第幾週、phase=階段、egiu=本週 EGIU 範圍、
//       items=[[skill, day, text], ...]，day: 0=週一…6=週日
// ============================================================
...
const IELTS_STUDY_PLAN = [
  ...
  { week: '2026-07-20', no: 1, phase: '第一階段・方法建立（W1–3）', egiu: 'Unit 1–7', items: [
    ['reading', 0, '平行閱讀法 第一章（三種語言重現）精讀'],
    ['speaking', 2, 'Cambly＋課後把語料丟收集箱'],
    ['writing', 4, 'Simon Task 2 四段結構＋13句法熟讀'],
    ['reading', 5, '平行閱讀法 第二章（題型分析）開始'],
    ['writing', 6, '🔄 機動日：每週回顧＋補做落後的'],
    ['material', 6, 'EGIU Unit 1–7'],
  ]},
  { week: '2026-07-27', no: 2, phase: '第一階段・方法建立（W1–3）', egiu: 'Unit 8–13', items: [
    ['reading', 0, '平行閱讀法 第二章（填空題）精讀'],
    ['reading', 1, '平行閱讀法 第二章 其餘題型'],
    ['speaking', 2, 'Cambly＋口說範例回答解析'],
    ['writing', 4, '寫作範文分析 1 篇'],
    ['reading', 5, '平行閱讀法 第二章 續'],
    ['writing', 6, '🔄 機動日：每週回顧＋補做落後的'],
    ['material', 6, 'EGIU Unit 8–13'],
  ]},
  { week: '2026-08-03', no: 3, phase: '第一階段・方法建立（W1–3）', egiu: 'Unit 14–20', items: [
    ['speaking', 0, '口說課 30 分鐘（Cambly 專門老師）'],
    ['reading', 0, '平行閱讀法 第二章 最後兩節'],
    ['reading', 1, '平行閱讀法 第三章（五步驟方法論）讀完，全書收尾'],
    ['speaking', 2, '口說課 30 分鐘＋課後語料整理'],
    ['writing', 4, '範文分析 1 篇＋抽語料進「Task 2 佳句」（不背整篇）'],
    ['listening', 5, '【第一次】劍10 T1 聽力完整一回＋六步驟檢討（約3小時，重點是找出弱點在哪）'],
    ['writing', 6, '🔄 機動日：每週回顧（平行閱讀法讀完了！下週開始做題）'],
    ['material', 6, 'EGIU Unit 14–20'],
  ]},

  // ===== 第二階段：應用方法・完整檢討（W4–9）——診斷期，不限時，閱讀 2 篇/週 =====
  { week: '2026-08-10', no: 4, phase: '第二階段・應用方法・完整檢討（W4–9）', egiu: 'Unit 21–27', items: [
    ['speaking', 0, '口說課 30 分鐘'],
    ['reading', 0, '劍10 T1 P1 不限時＋拆解檢核（每題都過）'],
    ['reading', 1, '劍10 T1 P2 不限時＋拆解檢核'],
    ['speaking', 2, '口說課 30 分鐘＋建立第1份核心語料'],
    ['writing', 4, '範文分析＋plan 練習×2 題（只規劃不寫全篇）'],
    ['listening', 5, '劍10 T2 聽力完整一回＋六步驟檢討'],
    ['writing', 6, '🔄 機動日：每週回顧＋補做落後的'],
    ['material', 6, 'EGIU Unit 21–27'],
  ]},
  { week: '2026-08-17', no: 5, phase: '第二階段・應用方法・完整檢討（W4–9）', egiu: 'Unit 28–34', items: [
    ['speaking', 0, '口說課 30 分鐘'],
    ['reading', 0, '劍10 T1 P3 不限時＋拆解檢核'],
    ['reading', 1, '劍10 T2 P1 不限時＋拆解檢核'],
    ['speaking', 2, '口說課 30 分鐘＋核心語料第2份'],
    ['writing', 4, '範文分析＋plan 練習×2 題'],
    ['listening', 5, '劍10 T3 聽力完整一回＋檢討'],
    ['writing', 6, '🔄 機動日：每週回顧＋補做落後的'],
    ['material', 6, 'EGIU Unit 28–34'],
  ]},
  { week: '2026-08-24', no: 6, phase: '第二階段・應用方法・完整檢討（W4–9）', egiu: 'Unit 35–41', items: [
    ['speaking', 0, '口說課 30 分鐘'],
    ['reading', 0, '劍10 T2 P2 不限時＋拆解檢核'],
    ['reading', 1, '劍10 T2 P3 不限時＋拆解檢核'],
    ['speaking', 2, '口說課 30 分鐘＋自錄 15 分鐘'],
    ['writing', 4, 'Task 1 結構＋圖表詞彙＋範文分析'],
    ['listening', 5, '劍10 T4 聽力完整一回＋檢討'],
    ['writing', 6, '🔄 機動日：每週回顧＋補做落後的'],
    ['material', 6, 'EGIU Unit 35–41'],
  ]},
  { week: '2026-08-31', no: 7, phase: '第二階段・應用方法・完整檢討（W4–9）', egiu: 'Unit 42–45, 49, 50, 53', items: [
    ['speaking', 0, '口說課 30 分鐘'],
    ['reading', 0, '劍10 T3 P1 不限時＋拆解檢核'],
    ['reading', 1, '劍10 T3 P2 不限時＋拆解檢核'],
    ['speaking', 2, '口說課 30 分鐘＋核心語料第3份'],
    ['writing', 4, '最後一週 plan 練習×3 題（下週開始寫完整篇）'],
    ['listening', 5, '劍11 T1 聽力完整一回＋檢討'],
    ['writing', 6, '🔄 機動日：每週回顧＋補做落後的'],
    ['material', 6, 'EGIU Unit 42–45, 49, 50, 53'],
  ]},
  { week: '2026-09-07', no: 8, phase: '第二階段・應用方法・完整檢討（W4–9）', egiu: 'Unit 54–58, 60, 62', items: [
    ['speaking', 0, '口說課 30 分鐘'],
    ['reading', 0, '劍10 T3 P3 不限時＋拆解檢核'],
    ['reading', 1, '劍10 T4 P1 不限時＋拆解檢核'],
    ['speaking', 2, '口說課 30 分鐘＋自錄 15 分鐘'],
    ['writing', 4, '【第一篇完整作文】Task 2 不限時（plan 10 分鐘要做滿）'],
    ['listening', 5, '劍11 T2 聽力完整一回＋檢討'],
    ['writing', 6, '🔄 機動日：Task 2 訂正（訂正比多寫一篇有效）＋每週回顧'],
    ['material', 6, 'EGIU Unit 54–58, 60, 62'],
  ]},
  { week: '2026-09-14', no: 9, phase: '第二階段・應用方法・完整檢討（W4–9）', egiu: 'Unit 64–70', items: [
    ['speaking', 0, '口說課 30 分鐘'],
    ['reading', 0, '劍10 T4 P2 不限時＋拆解檢核'],
    ['reading', 1, '劍10 T4 P3＋劍10 全書錯題回顧'],
    ['speaking', 2, '口說課 30 分鐘＋核心語料第4份'],
    ['writing', 4, 'Task 1 一篇不限時＋對照範文自我檢查'],
    ['listening', 5, '劍11 T3 聽力完整一回＋檢討'],
    ['writing', 6, '🔄 機動日：Task 1 訂正＋階段回顧（劍10 做完，列出四科弱點）'],
    ['material', 6, 'EGIU Unit 64–70'],
  ]},

  // ===== 第三階段：限時＋精簡檢討（W10–13）——檢討開始收斂，只追錯題 =====
  { week: '2026-09-21', no: 10, phase: '第三階段・限時＋精簡檢討（W10–13）', egiu: 'Unit 71–77', items: [
    ['speaking', 0, '口說課 30 分鐘'],
    ['reading', 0, '劍11 T1 閱讀整回 60 分鐘限時（首次全限時）'],
    ['reading', 1, '劍11 T1 檢討：只對錯題＋「答對但用猜的」做拆解檢核'],
    ['speaking', 2, '口說課 30 分鐘＋自錄×2'],
    ['writing', 4, 'Task 2 一篇 60 分鐘內'],
    ['listening', 5, '劍11 T4 聽力整回 30 分＋精簡檢討（只精講錯題＋重聽一次）'],
    ['writing', 6, '🔄 機動日：Task 2 訂正＋每週回顧'],
    ['material', 6, 'EGIU Unit 71–77'],
  ]},
  { week: '2026-09-28', no: 11, phase: '第三階段・限時＋精簡檢討（W10–13）', egiu: 'Unit 78–84', items: [
    ['speaking', 0, '口說課 30 分鐘'],
    ['reading', 0, '劍11 T2 閱讀整回 60 分鐘限時'],
    ['reading', 1, '劍11 T2 檢討（只錯題＋猜對的）'],
    ['speaking', 2, '口說課 30 分鐘＋口說模考（Part 1–3 全程錄音）'],
    ['writing', 4, 'Task 1 30 分鐘限時'],
    ['listening', 5, '劍12 T1 聽力整回 30 分＋精簡檢討'],
    ['writing', 6, '🔄 機動日：每週回顧＋補做落後的'],
    ['material', 6, 'EGIU Unit 78–84'],
  ]},
  { week: '2026-10-05', no: 12, phase: '第三階段・限時＋精簡檢討（W10–13）', egiu: 'Unit 85–90, 92', items: [
    ['speaking', 0, '口說課 30 分鐘'],
    ['reading', 0, '劍11 T3 閱讀整回 60 分鐘限時'],
    ['reading', 1, '劍11 T3 檢討（只錯題＋猜對的）'],
    ['speaking', 2, '口說課 30 分鐘＋弱點題型練習'],
    ['writing', 4, '模板總複習（不寫新的，為明天模考留體力）'],
    ['reading', 5, '【模考1】劍18 T1 聽力＋閱讀＋寫作連做（照考試時序）'],
    ['writing', 6, '🔄 機動日：模考1 完整檢討（四科弱點清單）＋成績記進練習紀錄'],
    ['material', 6, 'EGIU Unit 85–90, 92'],
  ]},
  { week: '2026-10-12', no: 13, phase: '第三階段・限時＋精簡檢討（W10–13）', egiu: 'Unit 93–99', items: [
    ['speaking', 0, '口說課 30 分鐘'],
    ['reading', 0, '模考1 弱題型專練'],
    ['listening', 1, '模考1 聽力錯因專練＋場景詞'],
    ['speaking', 2, '口說課 30 分鐘＋弱點題型練習'],
    ['writing', 4, 'Task 2 弱題型一篇 40 分鐘限時（首次限時）'],
    ['listening', 5, '劍12 T2 聽力整回 30 分＋精簡檢討'],
    ['writing', 6, '🔄 機動日：Task 2 訂正＋每週回顧'],
    ['material', 6, 'EGIU Unit 93–99'],
  ]},

  // ===== 第四階段：模考衝刺（W14–17）——隔週一次全模考 =====
  { week: '2026-10-19', no: 14, phase: '第四階段・模考衝刺（W14–17）', egiu: 'Unit 100, 105–108, 113, 115', items: [
    ['speaking', 0, '口說課 30 分鐘'],
    ['reading', 0, '劍12 T1 閱讀整回 60 分鐘限時'],
    ['reading', 1, '劍12 T1 檢討（只錯題＋猜對的）'],
    ['speaking', 2, '口說課 30 分鐘＋口說模考第2次'],
    ['writing', 4, 'Task 1 各圖表模板總複習（不寫新的）'],
    ['reading', 5, '【模考2】劍18 T2 全科連做'],
    ['writing', 6, '🔄 機動日：模考2 完整檢討＋每週回顧'],
    ['material', 6, 'EGIU Unit 100, 105–108, 113, 115'],
  ]},
  { week: '2026-10-26', no: 15, phase: '第四階段・模考衝刺（W14–17）', egiu: 'Unit 116, 119–122, 128, 129', items: [
    ['speaking', 0, '口說課 30 分鐘'],
    ['reading', 0, '劍12 T2 閱讀整回 60 分鐘限時'],
    ['reading', 1, '劍12 T2 檢討＋模考2 弱點專練'],
    ['speaking', 2, '口說課 30 分鐘＋串題總演練（一份語料對五題）'],
    ['writing', 4, 'Task 2 高頻主題 plan 練習×3（只 plan 不寫全篇）'],
    ['listening', 5, '劍12 T3 聽力整回 30 分＋精簡檢討'],
    ['writing', 6, '🔄 機動日：每週回顧＋補做落後的'],
    ['material', 6, 'EGIU Unit 116, 119–122, 128, 129'],
  ]},
  { week: '2026-11-02', no: 16, phase: '第四階段・模考衝刺（W14–17）', egiu: 'Unit 130–136（EGIU 精選完成！）', items: [
    ['speaking', 0, '口說課 30 分鐘'],
    ['reading', 0, '弱題型專練（依模考2 弱點清單）'],
    ['listening', 1, '聽力錯因總瀏覽＋場景詞掃讀'],
    ['speaking', 2, '口說課 30 分鐘＋考場流程演練'],
    ['writing', 4, 'Task 2 高頻主題 plan 練習×3'],
    ['reading', 5, '【模考3】劍19 T1 全科連做'],
    ['writing', 6, '🔄 機動日：模考3 完整檢討＋每週回顧'],
    ['material', 6, 'EGIU Unit 130–136 → 精選 98 個單元全部完成！🎉'],
  ]},
  { week: '2026-11-09', no: 17, phase: '第四階段・模考衝刺（W14–17）', egiu: '總複習（只翻答錯過的單元）', items: [
    ['speaking', 0, '口說課 30 分鐘'],
    ['reading', 0, '模考3 弱點專練'],
    ['listening', 1, '輕量：Shadowing＋場景詞'],
    ['speaking', 2, '口說課 30 分鐘＋考場流程演練'],
    ['writing', 4, '模板默寫一次（Task 1＋2 各題型開頭結尾）'],
    ['reading', 5, '【模考4】劍19 T2 全科連做（最後一次全模考）'],
    ['writing', 6, '🔄 機動日：模考4 檢討＋拆解檢核紀錄總回顧（看平均分與猜題率）'],
    ['material', 6, 'EGIU：只翻答錯過的單元'],
  ]},
  ...
  { week: '2026-11-16', no: 18, phase: '考前週（11/21 週六考試）', egiu: '考前休息，不排新單元', items: [
    ...
    ['speaking', 2, '口說課（考前最後調整）＋語料快速過一遍'],
    ['writing', 4, '模板默寫一次＋準備考試用品、早睡'],
    ['reading', 5, '11/21（六）考試日！平行閱讀法五步驟心中過一遍　加油 🎉'],
    ['material', 6, '考完了！好好休息 🎊'],
  ]},
];
